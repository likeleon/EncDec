...
static int configure_x264_param( x264_param_t *param )
{
	if (!param)
		return -1;
	param->b_annexb = 0;
...
static int open_file( char *psz_filename, hnd_t *p_handle, cli_output_opt_t *opt )
{
    ...
    *p_handle = NULL;
    ...
    p_mkv  = static_cast<mkv_hnd_t *>(malloc( sizeof(*p_mkv) ));
    if( !p_mkv )
        return -1;
...
{
    mkv_hnd_t   *p_mkv = static_cast<mkv_hnd_t *>(handle);
    int64_t dw, dh;
...
static int write_headers( hnd_t handle, x264_nal_t *p_nal )
{
    ...
    avcC = static_cast<uint8_t*>(malloc( avcC_len ));
    if( !avcC )
        return -1;
...
const cli_output_t mkv_output = { configure_x264_param, open_file, set_param, write_headers, write_frame, close_file };
