...
static mk_context *mk_create_context( mk_writer *w, mk_context *parent, unsigned id )
{
    ...
    {
        c = static_cast<mk_context *>(malloc( sizeof(*c) ));
        if( !c )
            return NULL;
        memset( c, 0, sizeof(*c) );
    }
...
{
    mk_writer *w = static_cast<mk_writer *>(malloc( sizeof(*w) ));
    if( !w )
        return NULL;
...
static inline uint8_t is_regular_file( FILE *filehandle )
{
...
int mk_close( mk_writer *w, int64_t last_delta )
{
    ...
    if( mk_flush_frame( w ) < 0 || mk_close_cluster( w ) < 0 )
        ret = -1;
    if( w->wrote_header && is_regular_file( w->fp ) )
    {
